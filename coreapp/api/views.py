...
from ..utils import auth_utils,forget_utils
...
class ForgetPasswordAPI(APIView):
    ...
    def post(self, request):
        serializer = serializers.ForgetPassSerializer(
            data=request.data, context={"request": self.request})
        if serializer.is_valid():
            email = serializer.validated_data['email']
            data = forget_utils.process_code(email)
...
class ForgetPasswordConfirmAPI(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            password = serializer.validated_data['password']
            confirmation = forget_utils.get_code(email, code)
            if confirmation.confirmation_code == code:
                confirmation.is_used = True
                confirmation.save()
                user = forget_utils.get_user(email=email)
